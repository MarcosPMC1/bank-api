...
describe('PaymentsService', () => {
  ...
  describe('create', () => {
    ...
    it('cannot send value to yourself', () => {
      jest
        ...
        .mockRejectedValue(
          new BadRequestException('Cannot send value to yourself'),
        );
      const result = service.create({
        description: paymentMock.description,
        receiver: paymentMock.idreceiver,
    ...
    it('receiver not found', () => {
      jest
        .spyOn(dataSource, 'transaction')
        .mockRejectedValue(new NotFoundException('Receiver not found'));
      const result = service.create({
        description: paymentMock.description,
        receiver: '7',
        sender: paymentMock.idreceiver,
    ...
    it('sender not has balance', () => {
      jest
        .spyOn(dataSource, 'transaction')
        .mockRejectedValue(new BadRequestException('Sender not has balance'));
      const result = service.create({
        description: paymentMock.description,
        receiver: paymentMock.idreceiver,
        sender: paymentMock.idsender,
      ...
      expect(result).rejects.toThrow(
        new BadRequestException('Sender not has balance'),
      );
    });
  });
  ...
  describe('findOne', () => {
    it('should success', () => {
      jest
        .spyOn(paymentsRepository, 'findOneOrFail')
